...
export const MAX_WORD = new BN(2).pow(new BN(256)).sub(new BN(1));
 ...
 */
export function cleanBNs(value) {
  if (isBN(value)) {
...
export function toBN(bytes) {
  if (bytes == undefined) {
    return undefined;
  } else if (typeof bytes == "string") {
    return new BN(bytes, 16);
  } else if (typeof bytes == "number" || isBN(bytes)) {
    return new BN(bytes);
  } else if (bytes.reduce) {
    return bytes.reduce(
      (num, byte) => num.mul(new BN(0x100)).add(new BN(byte)),
      new BN(0)
    );
  }
}

export function toSignedBN(bytes) {
  if (bytes[0] < 0b10000000) {  // first bit is 0
    return toBN(bytes);
  } else {
    return toBN(bytes.map( (b) => new BN(0xff - b).add(new BN(1)).neg()));
  }
}
