...
export function decodeValue(definition, pointer, state, ...args) {
  ...
  switch (utils.typeClass(definition)) {
    case "bool":
      return !utils.toBN(bytes).isZero();

    case "uint":
      return utils.toBN(bytes);

    case "int":
      return utils.toSignedBN(bytes);
...
export function decodeMemoryReference(definition, pointer, state, ...args) {
  let rawValue = utils.toBN(read(pointer, state)).toNumber();
...
export function decodeStorageReference(definition, pointer, state, ...args) {
  ...
  switch (utils.typeClass(definition)) {
    case "array":
      ...
      length = utils.toBN(data).toNumber();
          ...
          let childFrom = pointer.storage.from.offset != undefined ?
            {
              ...
              ).add(utils.toBN(pointer.storage.from.offset)).toString(16)],
              offset: offset(i),
              index: index(i)
            } : {
    ...
    case "string":
      ...
      if (data[WORD_SIZE - 1] % 2 == 0) {
        ...
        length = utils.toBN(data).sub(utils.toBN(1)).div(utils.toBN(2)).toNumber();
